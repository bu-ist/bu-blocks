...
var setupSplide = function setupSplide() {
  var blocks = document.getElementsByClassName('wp-block-bu-blocks-slideshow');
  Array.from(blocks).map(function (block) {
    // Add Splide required classes.
    block.classList.add('splide');
    var track = block.querySelector('.wp-block-bu-blocks-slideshow-container');
    if (track) {
      track.classList.add('splide__track');
    }
    var list = block.querySelector('.wp-block-bu-blocks-slideshow-list');
    if (list) {
    ...
    var settings = buBlocksSlideshowSettings[blockType];
    if (!settings) {
      settings = buBlocksSlideshowSettings["default"];
    }
    if (settings) {
      new _splide["default"](block, settings).mount();
    }
  });
};
setupSplide();

},{"@babel/runtime/helpers/interopRequireDefault":1,"@splidejs/splide":2}]},{},[3]);
