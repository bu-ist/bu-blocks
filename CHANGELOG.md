...
- BU Stats Block: 5.7 upgrade related changes to SVG attributes caused block validation failures. This fixes it by adding a block deprecation to cover the previous way WP handled SVG elements in 5.4 and added attributes to the SVG element that are no longer added in 5.7. 
Fixes: https://github.com/bu-ist/bu-stats/issues/106

## 0.4.0

### WordPress 5.7 Compatibility. 
This is a major release and not backwards compatible with previous versions of WP. Many blocks are now updated to support Block API 2 and useBlockProps. 
A variety of fixes and bugs are included in this release. Themes should update and recompile if they import .scss partials from BU Blocks to get the updated styles.
Themes may have to adjust their styling to compensate for adjustments made to blocks. 

- Fix [#383](https://github.com/bu-ist/bu-blocks/issues/383)
