...
bu_blocks.collapsibleControl = ( function () {
	...
	var getGroupCollapsibleBlocks = function ( control ) {
		...
		var containers = group.querySelectorAll(
			'.js-wp-block-bu-collapsible'
		);
...
bu_blocks.photoessay = ( function () {
	...
	var findElements = function () {
		//find all the blocks
		var findBlocks = document.querySelectorAll(
...
bu_blocks.slideshow = ( function () {
	...
	var findElements = function () {
		...
		if ( elements.length > 0 ) {
			...
			for ( i = 0; i < elements.length; i++ ) {
				...
				//get back btn
				block.backBtn = elements[ i ].getElementsByClassName(
					'js-bu-blocks-slideshow-back-btn'
				)[ 0 ];
				//get forward btn
				block.forwardBtn = elements[ i ].getElementsByClassName(
					'js-bu-blocks-slideshow-forward-btn'
				)[ 0 ];

				//get onmedia back btn
				block.backMediaBtn = elements[ i ].getElementsByClassName(
					'js-bu-blocks-slideshow-back-onmedia-btn'
				)[ 0 ];
				//get onmedia forward btn
				block.forwardMediaBtn = elements[ i ].getElementsByClassName(
					'js-bu-blocks-slideshow-forward-onmedia-btn'
				)[ 0 ];
				//get ontrack forward btn
				block.forwardTrackBtn = elements[ i ].getElementsByClassName(
					'js-bu-blocks-slideshow-forward-ontrack-btn'
				)[ 0 ];

				//get caption btn
				block.captionBtn = elements[ i ].getElementsByClassName(
					'js-bu-blocks-slideshow-caption-btn'
				)[ 0 ];

				//get media track
				block.mediatrack = elements[ i ].getElementsByClassName(
					'js-bu-blocks-slideshow-media-track'
				)[ 0 ];

				//get media items
				block.mediatrackitems = elements[ i ].getElementsByClassName(
					'js-bu-blocks-slideshow-media-track-item'
				);

				//get caption track
				block.captiontrack = elements[ i ].getElementsByClassName(
					'js-bu-blocks-slideshow-caption-track'
				)[ 0 ];

				//get caption items
				block.captiontrackitems = elements[ i ].getElementsByClassName(
					'js-bu-blocks-slideshow-caption-item'
				);
				//get caption items
				block.captionContainer = elements[ i ].getElementsByClassName(
					'js-bu-blocks-slideshow-caption-container'
				)[ 0 ];
			...
			for ( var i = block.mediatrackitems.length - 1; i >= 0; i-- ) {
				...
				if ( block.mediatrackitems[ i ].active === true ) {
					block.mediatrackitems[ 0 ].classList.toggle(
						'bu-blocks-slideshow-media-track-item-active'
					);
					block.captiontrackitems[ 0 ].classList.toggle(
						'bu-blocks-slideshow-media-track-item-active'
					);
				}
			}
	...
	var setupHandlers = function () {
		if ( slideshowBlocks.length > 0 ) {
			for ( i = 0; i < slideshowBlocks.length; i++ ) {
				...
				block.forwardMediaBtn.addEventListener(
					'click',
					function ( e ) {
						e.preventDefault();
						nextItem( block );
				...
				block.forwardTrackBtn.addEventListener(
					'click',
					function ( e ) {
						e.preventDefault();
						nextItem( block );
	...
	var setupMediaTrack = function () {
		for ( i = 0; i < slideshowBlocks.length; i++ ) {
			...
			if ( block.mediatrack && block.mediatrackitems ) {
				...
				//set default width
				block.mediatrack.style.width =
					'calc(' + block.itemslength + ' * 100%)';
				...
				for ( var i = 0; i < block.mediatrackitems.length; i++ ) {
					block.mediatrackitems[ i ].style.width =
						'calc(1/' + block.itemslength + ' * 100% - 2px)';
				}
			}
			...
			if ( block.captiontrack && block.captiontrackitems ) {
				//set default width
				block.captiontrack.style.width =
					'calc(' + block.itemslength + ' * 100%)';
				...
				for ( var i = 0; i < block.captiontrackitems.length; i++ ) {
					block.captiontrackitems[ i ].style.width =
						'calc(1/' + block.itemslength + ' * 100%)';
				}
			}
		}
	};
	...
	var nextItem = function ( block ) {
		if ( block.currentItem === block.itemslength - 1 ) {
			...
			for ( var i = block.mediatrackitems.length - 1; i >= 0; i-- ) {
				if ( block.mediatrackitems[ i ].active === true ) {
					block.mediatrackitems[ i ].active = false;
					block.mediatrackitems[ i ].classList.toggle(
						'bu-blocks-slideshow-media-track-item-active'
					);
					block.mediatrackitems[ i + 1 ].active = true;
					block.mediatrackitems[ i + 1 ].classList.toggle(
						'bu-blocks-slideshow-media-track-item-active'
					);

					block.captiontrackitems[ i ].active = false;
					block.captiontrackitems[ i ].classList.toggle(
						'bu-blocks-slideshow-media-track-item-active'
					);
					block.captiontrackitems[ i + 1 ].active = true;
					block.captiontrackitems[ i + 1 ].classList.toggle(
						'bu-blocks-slideshow-media-track-item-active'
					);
				}
			}
	...
	var toggleCaption = function ( block ) {
		...
		block.captionContainer.classList.toggle(
			...
			if ( block.captionContainer.collapsed == false ) {
				if ( block.captiontrackitems[ i ].active === true ) {
					block.captiontrack.style.maxHeight =
						block.captiontrackitems[ i ].offsetHeight + 'px';
				}
			} else {
				...
				if ( block.captiontrackitems[ i ].active === true ) {
					...
					if (
						block.captiontrackitems[ i ].offsetHeight <
						block.captiontrack.offsetHeight
					) {
						block.captionContainer.classList.remove(
							'bu-blocks-slideshow-caption-container-collapsed'
						);
						block.captionBtn.style.display = 'none';
					} else {
						block.captionContainer.classList.add(
							'bu-blocks-slideshow-caption-container-collapsed'
						);
